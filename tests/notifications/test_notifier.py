...
def test_decrypt_config_rejects_plaintext(db_engine):
    ...
    crypto = MagicMock()
    notifier = Notifier(db_engine, channels={'bark': bark}, crypto=crypto)
    notifier.notify_path_a(comparison_id=cmp.id, lottery_name='双色球', draw_no='062', tier=1, amount=None)
    # 明文配置被拒绝 → 渠道不被调用
    bark.send.assert_not_called()
...
def test_decrypt_config_logs_on_failure(db_engine, caplog):
    ...
    # 解密失败必须留痕（非静默吞掉）
    assert any(
        'decrypt' in rec.message.lower() or '解密' in rec.message or 'config' in rec.message.lower()
...
def test_sent_at_is_naive_utc_aligned_with_created_at(db_engine):
    ...
    crypto.decrypt.return_value = '{"key":"k","url":"https://api.day.app"}'
    notifier = Notifier(db_engine, channels={'bark': bark}, crypto=crypto)
    before = _dt.now(UTC).replace(tzinfo=None)
    notifier.notify_path_a(comparison_id=cmp_id, lottery_name='双色球', draw_no='062', tier=1, amount=None)
    after = _dt.now(UTC).replace(tzinfo=None)
...
def test_no_user_channels_does_not_trigger_admin_alert(monkeypatch, db_engine, caplog):
    ...
    from app.models import Comparison, DrawResult, NotificationRule, User
    from app.seeds.lottery_types import seed_lottery_types

...
def test_draw_result_columns(db_engine):
    assert _EXPECTED_DRAW_RESULT_COLS.issubset(_cols(db_engine, 'draw_results')), (
...
def test_comparison_columns(db_engine):
    ...
    assert _EXPECTED_COMPARISON_COLS.issubset(actual), f'comparisons 缺列: {_EXPECTED_COMPARISON_COLS - actual}'


def test_prize_claim_columns(db_engine):
    actual = _cols(db_engine, 'prize_claims')
    assert _EXPECTED_PRIZE_CLAIM_COLS.issubset(actual), f'prize_claims 缺列: {_EXPECTED_PRIZE_CLAIM_COLS - actual}'
...
def test_draw_results_unique_lottery_code_draw_no(db_engine):
    uqs = {tuple(c['column_names']) for c in inspect(db_engine).get_unique_constraints('draw_results')}
    assert ('lottery_code', 'draw_no') in uqs


def test_comparisons_unique_draw_result_ticket(db_engine):
    uqs = {tuple(c['column_names']) for c in inspect(db_engine).get_unique_constraints('comparisons')}
...
def test_defaults(db_engine):
    ...
    pend_cols = {c['name']: c for c in inspect(db_engine).get_columns('pending_comparisons')}
...
def test_raw_sql_insert_honors_defaults(db_engine):
    ...
    with db_engine.begin() as conn:
        ...
        row = conn.execute(text('SELECT multiplier, append, enabled, cost FROM tickets WHERE id = 1')).one()
        ...
        dr = conn.execute(text('SELECT verified, single_source, version FROM draw_results WHERE id = 1')).one()
...
def test_full_compare_writeback_roundtrip(db_engine):
    ...
    with Session(db_engine) as s:
        ...
        draw = DrawResult(
            lottery_code='ssq',
            draw_no='2026062',
            draw_date=datetime(2026, 6, 22, tzinfo=UTC),
        ...
        s.add(
            DrawCorrection(
                draw_result_id=draw.id,
                old_numbers_json='{"back":[7]}',
                new_numbers_json='{"back":[8]}',
                reason='官方更正',
            )
        )
        s.commit()

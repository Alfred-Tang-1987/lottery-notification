...
def test_ticket_repo_list_by_lottery(db_engine):
    with Session(db_engine) as s:
        ...
        repo.create(lottery_code="ssq", play_type="single",
                    numbers_json='{"front":[1,2,3,4,5,6],"back":[7]}', cost=200)
        repo.create(lottery_code="dlt", play_type="single",
                    numbers_json='{"front":[1,2,3,4,5],"back":[1,2]}', cost=200)
        repo.create(lottery_code="ssq", play_type="single",
                    numbers_json='{"front":[2,3,4,5,6,7],"back":[8]}', cost=200, enabled=False)
...
def test_ticket_repo_update(db_engine):
    with Session(db_engine) as s:
        u = _make_user(s, "u1")
        repo = TicketRepo(s, user_id=u.id)
        t = repo.create(lottery_code="ssq", play_type="single",
                        numbers_json='{"front":[1,2,3,4,5,6],"back":[7]}', cost=200)

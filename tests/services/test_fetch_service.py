...
def _dn(code, no, front, back=None, draw_date=date(2026, 6, 21)):
    ...
    return DrawNumbers(
        lottery_code=code, draw_no=no, draw_date=draw_date,
...
def test_fetch_both_sources_failed(db_engine):
    ...
    svc = FetchService(primary, backup, db_engine, max_attempts=2, backoff_base=0,
                       sleep=lambda *_: None)
    r = svc.fetch_and_store("ssq")
...
def test_store_writes_pending_comparison_on_verified(db_engine):
    ...
    with Session(db_engine) as s:
        dr = s.exec(select(DrawResult)).first()
        pc = s.exec(select(PendingComparison).where(
            PendingComparison.draw_result_id == dr.id)).first()
...
def test_idempotent_upgrades_single_source_to_dual(db_engine):
    ...
    # 第一次：主源有、备源无 → 单源
    svc1 = FetchService(_src(dn, name="mxnzp"), _src(None, name="juhe"),
                        db_engine, grace_seconds=0)
    r1 = svc1.fetch_and_store("ssq")
    assert r1.stored and r1.verified and r1.single_source
    # 第二次：双源一致 → 升级
    svc2 = FetchService(_src(dn, name="mxnzp"), _src(dn, name="juhe"),
                        db_engine, grace_seconds=0)
...
def test_draw_date_is_official_not_fetch_time(db_engine):
    ...
    primary = _src(_dn("ssq", "062", [1, 2, 3, 4, 5, 6], [7], draw_date=official),
                   name="mxnzp")
    backup = _src(_dn("ssq", "062", [1, 2, 3, 4, 5, 6], [7], draw_date=official),
                  name="juhe")
    FetchService(primary, backup, db_engine, grace_seconds=0).fetch_and_store("ssq")
    with Session(db_engine) as s:
        dr = s.exec(select(DrawResult)).first()
        assert dr.draw_date.date() == official, (
            f"draw_date 必须是官方开奖日 {official}，得到 {dr.draw_date.date()}")
...
def test_store_verified_and_outbox_atomic(db_engine):
    ...
    with Session(db_engine) as s:
        ...
        pcs = s.exec(select(PendingComparison).where(
            PendingComparison.draw_result_id == drs[0].id)).all()
...
def test_upgrade_does_not_bless_changed_numbers(db_engine):
    ...
    # 第一次：主源单源（旧号码）入库
    svc1 = FetchService(_src(old_dn, name="mxnzp"), _src(None, name="juhe"),
                        db_engine, grace_seconds=0)
    svc1.fetch_and_store("ssq")
    # 第二次：双源返回新号码 → 不应 bless 旧号码
    svc2 = FetchService(_src(new_dn, name="mxnzp"), _src(new_dn, name="juhe"),
                        db_engine, grace_seconds=0)
    r2 = svc2.fetch_and_store("ssq")
    with Session(db_engine) as s:
        dr = s.exec(select(DrawResult)).first()

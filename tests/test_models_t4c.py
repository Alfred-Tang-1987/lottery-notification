...
_EXPECTED_NOTIFICATION_CHANNEL_COLS = {
    "id", "user_id", "type", "config_json", "enabled", "key_version", "created_at",
}
_EXPECTED_NOTIFICATION_RULE_COLS = {
    "id", "user_id", "lottery_code", "strategy", "timing", "created_at",
}
_EXPECTED_NOTIFICATION_LOG_COLS = {
    "id", "user_id", "type", "payload", "status", "sent_at", "error", "created_at",
}
_EXPECTED_API_SOURCE_HEALTH_COLS = {
    "source", "last_success_at", "status", "error", "created_at",
}
...
_EXPECTED_APSCHEDULER_JOB_COLS = {
    "id", "next_run_time", "job_state", "created_at",
}
...
def test_notification_channel_columns(db_engine):
    assert _EXPECTED_NOTIFICATION_CHANNEL_COLS.issubset(
        _cols(db_engine, "notification_channels")
...
def test_notification_rule_columns(db_engine):
    assert _EXPECTED_NOTIFICATION_RULE_COLS.issubset(
        _cols(db_engine, "notification_rules")
...
def test_notification_log_columns(db_engine):
    assert _EXPECTED_NOTIFICATION_LOG_COLS.issubset(
        _cols(db_engine, "notification_logs")
...
def test_api_source_health_columns(db_engine):
    assert _EXPECTED_API_SOURCE_HEALTH_COLS.issubset(
        _cols(db_engine, "api_source_health")
...
def test_admin_audit_log_columns(db_engine):
    assert _EXPECTED_ADMIN_AUDIT_LOG_COLS.issubset(
        _cols(db_engine, "admin_audit_logs")
...
def test_apscheduler_job_columns(db_engine):
    assert _EXPECTED_APSCHEDULER_JOB_COLS.issubset(
        _cols(db_engine, "apscheduler_jobs")
...
def test_full_notification_writeback_roundtrip(db_engine):
    ...
    with Session(db_engine) as s:
        ...
        ch = NotificationChannel(
            user_id=user.id, type="bark", config_json='{"key":"abc"}',
        ...
        rule = NotificationRule(
            user_id=user.id, lottery_code=lottery.code,
            strategy="win_only", timing="07:00",
        )
        s.add(rule)
        s.commit()
        ...
        # log
        log = NotificationLog(
            user_id=user.id, type="bark", payload="test", status="sent",
            sent_at=datetime(2026, 6, 22, 7, 0, tzinfo=timezone.utc),
        )
        s.add(log)
        s.commit()
...
def test_health_and_audit_writeback(db_engine):
    ...
    with Session(db_engine) as s:
        # health
        h = ApiSourceHealth(
            source="mxnzp", status="ok",
            last_success_at=datetime(2026, 6, 22, 21, 30, tzinfo=timezone.utc),
        )
        s.add(h)
        s.commit()
        ...
        user = s.exec(select(User)).one()
        audit = AdminAuditLog(
            admin_id=user.id, action="verify_draw", target_type="draw_result",
            target_id="1", old_values='{"verified":false}', new_values='{"verified":true}',
        )
        s.add(audit)
        s.commit()
...
def test_apscheduler_job_writeback(db_engine):
    ...
    with Session(db_engine) as s:
        ...
        job = ApschedulerJob(
            id="fetch_ssq_20260622", next_run_time=datetime(2026, 6, 22, 21, 30, tzinfo=timezone.utc),
            job_state=job_state,
        )
        s.add(job)
        s.commit()

...
| **开奖获取** | 双源容灾架构（MXNZP 主 + 备源可插拔）、定时 + 手动、多源交叉校验。**当前备源 Juhe 已废弃，单源运行中，待接入新备源（见 §7.2 双源现状）** |
...
一源成功一源「未开奖」 → 等待 grace window(默认5分钟) 后重试；仍只有一源 → 单源入库,
                          verified=true, single_source=true, 健康面板标黄, 文案提示"单源校验"
一源成功一源「故障」   → 跳过 grace，立即单源入库（故障不会因等待自愈，grace 只服务数据延迟）
```

> **⚠️ 双源现状（2026-07-21 冒烟后更新）**：备源「聚合数据（Juhe）」彩票接口实测已废弃（数据停在 2019，不可用）。当前生产**实际为单源 MXNZP 运行**——主源成功即 `verified=true, single_source=true`。双源交叉校验的**代码架构完整保留**（`FetchService` 双源编排、交叉校验、mismatch 拒绝、grace 窗口、force-verify），待接入新备源（候选：ThinkAPI / 极速API / 京东万象）即可恢复真双源，无需改架构。降级期间：
> - 「双源安全网」承诺**暂停**——MXNZP 数据错误会静默入库（无第二源兜底）。运维需关注 MXNZP 服务健康（健康面板）+ 官方更正流程（§7.1 `DrawCorrectService`）作为补偿。
> - 单源 `verified=true` 的语义从「双源交叉一致」降级为「单源成功获取」——`single_source=true` 字段是判别依据，UI 文案需如实反映。
> - 接入新备源后，`single_source=false` 的行才代表「双源交叉校验通过」。

- **"空结果" 的语义**：若返回为空但 HTTP 成功，视为"该期尚未开奖"，标记 `not_drawn`，停止本轮，等下次轮询；不是错误。
- **部分源 grace window 的触发条件**：仅在「缺失源是未开奖（HTTP 成功但无数据，属数据延迟）」时等待 5 分钟重抓——给数据慢的源一个补齐窗口。**缺失源若是故障（HTTP 异常/超时/鉴权失败），跳过 grace 直接单源入库**——故障不会因等 5 分钟自愈，sleep 只白白阻塞启动/cron（NAS 场景 healthcheck 超时 → restart 循环）。单源结果在 UI 用黄色"单源校验"标签提示。
- **期号语义映射**：MXNZP 与聚合数据的 `draw_no` 命名可能不同（如 `2026062` vs `062`）或一方滞后一期；适配层先做**归一化期号映射**再交叉校验号码，否则"号码一致"无意义。
- **抓取退避/抖动**：每轮失败（timeout/429）用指数退避 + 随机抖动重试，单期最多 ~6 次；避免 15min 固定节奏把限流源一夜锤 ~100 次（7 彩种）触发封禁。MXNZP 普通会员 QPS=1，多彩种串行抓取需 ≥1s 间隔。
...
- **路径 B 次日汇总**：标题 `兑奖了吗 · {日期} 核对汇总`，正文「本期共核对 {N} 个追投彩种，中奖 {M} 笔：{逐笔 彩种 奖级 金额}；未中奖彩种（{X}）：{彩种名列表}。」其中 {N}/{X} 为**彩种数**（非注数），{M} 为中奖笔数；未中奖彩种按 every 策略列名单（win_only 无中奖不推）。"点击查看明细"待 SITE_URL + 渠道 click URL 落地页就绪后再恢复（Bark 纯文本无跳转目标，不空头承诺）。
...
| 个人维护 API（MXNZP）停服 | 原设计「聚合数据商业级备源」已失效（Juhe 彩票接口 2019 后停更，见 §7.2 双源现状）；当前为单源运行，MXNZP 停服 = 无开奖数据。缓解：健康面板告警 + 本地缓存最近一期兜底；长期需接入新备源（ThinkAPI / 极速API / 京东万象） |

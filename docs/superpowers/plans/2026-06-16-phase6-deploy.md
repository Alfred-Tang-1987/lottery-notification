...
**Goal:** 将完整系统（后端 + 前端静态）打包为单 Docker 镜像，以 docker-compose 部署到 NAS（FnOS, 192.168.8.168），端口 8280，`restart: always`（FnOS 关机坑），SQLite 持久化，配置走 .env。
...
## 前置
- NAS: 192.168.8.168（FnOS），SSH: `ssh -i ~/.ssh/fn_nas admin@192.168.8.168`
- 已装 Docker + docker compose

## 部署步骤
1. 上传项目到 NAS：
   `ssh admin@192.168.8.168 "mkdir -p /vol1/1000/Docker/lottery-notification"`
   `rsync -av --exclude data --exclude .env ./ admin@192.168.8.168:/vol1/1000/Docker/lottery-notification/`
2. SSH 进 NAS，配置 .env：
...
4. 验证：
   ...
   浏览器开 http://192.168.8.168:8280 → 登录页
...
```bash
# 1. 健康检查
curl http://192.168.8.168:8280/api/health
# 2. 注册首个 admin 用户（首次手动设 admin）
curl -X POST http://192.168.8.168:8280/api/auth/register \
...
"
# 4. 触发一次开奖获取比对
curl -X POST http://192.168.8.168:8280/api/admin/trigger?lottery_code=ssq \
...
- [ ] `curl http://192.168.8.168:8280/api/health` 返回 ok
- [ ] 浏览器登录 → 仪表盘可见
- [ ] 注册首个用户 → 提为 admin

...
// DO NOT EDIT — generated from workflow-engine@c1974ae6cabb3a936987162470f1088b5e9695d040fcdb25e70434bef0a1e288 by sync.mjs
// 源文件：src/index.js
// 生成命令：npm run build
...
3. For each {{plansDir}}/*.md: if frontmatter (starts with ---) read task models; else generate \u2014 extract LEAF ids \u2014 **CRITICAL: \u5FC5\u987B\u8FD4\u56DE frontmatter models: \u7684\u6BCF\u4E00\u4E2A key\uFF08\u542B\u6700\u5927\u7684 N\uFF0C\u5982 T10\uFF09\uFF0C\u4E00\u4E2A\u4E0D\u6F0F\uFF1Bbody \u91CC ## Task N \u82E5\u6709 ### Task NX \u5B50 task \u2192 \u53EA\u53D6\u5B50 task\uFF08NX\uFF09\uFF0C\u5B50 task \u4E0D\u53EF\u9057\u6F0F\uFF1B## Task N \u65E0\u5B50 task \u2192 \u53D6 N \u672C\u8EAB**\uFF08leaf-first: ## Task N with ### Task NX children \u2192 only NX; else N), modelHint (title contains \u5B89\u5168|\u52A0\u5BC6|\u8BA4\u8BC1|JWT|CSRF|Fernet|\u7B97\u6CD5|\u6BD4\u5BF9|\u7B56\u7565|\u8FB9\u754C|\u96C6\u6210|\u63A5\u53E3 \u2192 opus, else omit), write frontmatter at file top. Idempotent. Record each plan's file (full path) and seq (last two digits of filename, e.g. 01). Also read write_files from frontmatter if present (format: "write_files:
  ...
  - silent-failure
  - test-strategy"). Return per task as "lesson_categories" array (absent \u2192 empty array). Also read each task's "Type" field from frontmatter (if present), normalize via trim().toLowerCase(), and scan the task's brief text for refactor keywords (\u7528 AUDIT_REFACTOR_KEYWORDS \u6B63\u5219). audit_required = (type === 'refactor') OR (brief matches keyword regex). Return per task as "audit_required" (boolean, default false).
...
5. git status --porcelain \u2192 dirty_tree. If dirty_tree=true (uncommitted changes from a crashed previous run, \xA76.2 \u534A\u63D0\u4EA4\u81EA\u6108), classify and handle each change (W1-1/W1-4, 2026-07-07):
...
// src/prompts/templates/spec-reviewer.md
var spec_reviewer_default = `You are the SPEC-REVIEWER (model opus). Verify implementor built EXACTLY what was requested \u2014 nothing missing, nothing extra, no misunderstanding. Verdict on CURRENT working tree (HEAD or staged).
...
var PROMPTS = {
  ...
  specReview: spec_reviewer_default,
  qualityReviewer: quality_reviewer_default,
...
var SCHEMAS = {
  bootstrap: {
    ...
    properties: {
      ...
      evidence: {
        type: "object",
        required: ["config", "plans", "completed", "git_log_subjects", "dirty_tree", "in_progress", "failed_approaches", "task_lessons", "task_write_files"],
...
function recordReviewRound(state2, taskKey2, round, spec, qual, hunt) {
  state2.perTask[taskKey2].review_rounds = round;
  state2.perTask[taskKey2].files_touched_per_round.push(unionFiles(spec, qual, hunt));
  state2.perTask[taskKey2].review_history.push(summarizeReviewRound(round, spec, qual, hunt));
...
function decideReviewOutcome(state2, taskKey2, round, spec, qual, hunt, model, maxRounds, cfg, reviewReason, emptyFailedReason) {
  ...
  if (maxRounds === 0) {
    ...
    if (round >= budget) return { action: "halt", reason: "review_not_converging", diag: { round, budget, findings_history: state2.perTask[taskKey2].findings_history, spec: spec?.diagnostics, qual: qual?.diagnostics, hunt: hunt?.diagnostics } };
  } else if (round === maxRounds) {
    return { action: "halt", reason: "review max rounds", diag: { round, findings_history: state2.perTask[taskKey2].findings_history, spec: spec?.diagnostics, qual: qual?.diagnostics, hunt: hunt?.diagnostics } };
  }
...
async function runFixRound(taskKey2, plan, task, round, spec, qual, hunt, state2, cfg, implCtx, model, maxRounds, concerns, concernsHint) {
...
- \u4E0D\u8981\u7559\u5230\u4E0B\u4E00\u8F6E\uFF0C\u4E0B\u4E00\u8F6E\u4E0D\u518D\u6709\u5347\u7EA7\u7A7A\u95F4
- \u622A\u81F3 r${round} review \u7D2F\u8BA1\u672A\u4FEE findings \u5982\u4E0A` : `\u4FEE\u590D review round ${round} \u95EE\u9898\uFF08${findings.length} \u9879\u53D1\u73B0\uFF1B\u2605 \u6807\u672C\u8F6E\u65B0\u589E\uFF09\u3002`;

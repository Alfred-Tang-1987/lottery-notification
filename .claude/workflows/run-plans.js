...
// reviewer 来源三元组（S4, 2026-07-07）：消除 collectReviewFindings/reviewHaltForEmptyFailed/summarizeReviewRound 三处硬编码。—— inline 自 lib.js
...
// formatFindingItem（S6, 2026-07-07）：统一 finding 格式化，消除 formatFindings/formatCrossReviewerNote 重复。—— inline 自 lib.js
...
// formatBulletSection（S5, 2026-07-07）：通用 bullet section 渲染，6 个 format* 复用。—— inline 自 lib.js
...
function ensurePerTaskDefaults(entry) {
  return {
    planId: null, status: 'in_progress', model: 'sonnet', audit_required: false, review_rounds: 0,
...
async function runTask(plan, task) {
  ...
  // 局限：fallback 正则只扫 title/model 等 frontmatter 可得字段，不扫 plan 文件 body 内的 brief。
  //   bootstrap（LLM）读完整 brief 才是主路径。
